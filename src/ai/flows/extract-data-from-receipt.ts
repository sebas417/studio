...
const ExtractDataOutputSchema = z.object({
  date: z.string().describe('The primary date of service, transaction, or purchase from the document, formatted strictly as YYYY-MM-DD. Example: 2024-03-15.'),
...
- Date of Service/Transaction: Identify the primary date of service or transaction on the document. If multiple dates are present (e.g., print date, due date, order date), prioritize the actual service, purchase, or transaction date. Ensure the date is formatted strictly as YYYY-MM-DD. For example, if the receipt shows 'March 15, 2024' as the transaction date, you should output '2024-03-15'. If the year is not explicitly mentioned but is clearly the current year or can be inferred, use that year.
